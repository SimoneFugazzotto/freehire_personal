...
// every open, non-private canonical job becomes an index document. Category resolution
// and a posting body help ranking and presentation, but are never catalogue gates:
// All Jobs must remain exhaustive (PRODUCT_VISION.md). Closed, private, and duplicate
// rows become deletions so they leave the index.
func splitJobs(jobs []db.Job, lookup realityLookup, geo closureGeoLookup, now time.Time) ([]search.JobDocument, []int64, error) {
	...
	for _, j := range jobs {
		// Only rows that must be absent from the public catalogue leave the index.
