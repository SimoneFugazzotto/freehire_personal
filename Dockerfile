...
RUN CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o /out/hire ./cmd/server \
 && CGO_ENABLED=0 GOOS=linux go build -ldflags="-s -w" -o /out/ \
      ./cmd/ingest ./cmd/harvest-ats ./cmd/harvest-boards ./cmd/add-board ./cmd/enrich ./cmd/reindex ./cmd/tg-ingest ./cmd/tg-extract \
...
RUN apt-get update \
 ...
 && pdftotext -v
COPY --from=build /out/harvest-ats /out/harvest-boards /out/hire /out/add-board /out/ingest /out/enrich /out/reindex /out/tg-ingest /out/tg-extract /out/backfill-derive /out/liveness /out/notify /out/import-collections /out/recount-companies /out/migrate /app/
